...
export function BurnArt({
  ...
  origin?: BurnOrigin
  /**
   * 'cover' eats the page from the origin outward. 'clear' eats the char away
   * again, so the reveal is the same fire continuing rather than a fade.
   */
  mode?: 'cover' | 'clear'
  className?: string
  ...
  const cut = progress
  /*
   * Unique per instance. SVG ids are document-global, so two burns on one page
   * sharing an id means the second silently renders the first one's gradients —
   * which looks exactly like the burn refusing to progress.
   */
        ...
        >
          {mode === 'cover' ? (
            ...
            <>
              ...
              <stop offset={pct(cut + 0.012)} stopColor="var(--burn-soot, #1e1008)" stopOpacity="0.82" />
              <stop offset={pct(cut + 0.03)} stopColor="var(--burn-char, #0a0705)" stopOpacity="0.96" />
              <stop offset="100%" stopColor="var(--burn-char, #0a0705)" stopOpacity="0.98" />
            </>
          )}
        </radialGradient>
      </defs>
