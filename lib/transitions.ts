...
export default function RootLayout({ children }) {
  return (
    ...
    props: [
      ...
      { name: 'paper', type: '"chalk" | "kraft" | "newsprint" | "ink" | "page" | "custom"', def: '"chalk"', description: 'Paper colour. "page" takes the current page\u2019s background, so it is the page itself that tears. "custom" applies no preset, leaving --tear-paper and --tear-fibre to you.' },
      { name: 'fringe', type: 'number', def: '22', description: 'Length of the fibrous fringe along the torn edge, in px. 0 for a clean cut.' },
      { name: 'quality', type: '"auto" | "high" | "low"', def: '"auto"', description: '"low" drops the shadow, the bump and some of the mesh for weaker devices. "auto" picks by device.' },
    ],
    controls: [
      { kind: 'select', key: 'paper', label: 'Paper', options: ['chalk', 'kraft', 'newsprint', 'ink', 'page'], def: 'chalk' },
      { kind: 'select', key: 'quality', label: 'Quality', options: ['auto', 'high', 'low'], def: 'auto' },
      { kind: 'range', key: 'fringe', label: 'Fringe', min: 0, max: 40, step: 1, def: 22 },
    ...
    notes: [
      ...
      'The seam starts above where you clicked and leans towards that point on its way down, and the tear catches and rips in fits the way paper does.',
      'Paper keeps its creases: once torn, a fold does not spring back out.',
