...
export const transitions: TransitionMeta[] = [
  {
    ...
    description:
      'The outgoing page leans away and drops back while the incoming page is unmasked over it from whichever edge it arrives at. It recedes into the edge it is anchored to rather than sliding off, so it stays visible behind the arriving page the whole way, and a few degrees of rotateX make that read as depth rather than a shrink. Both halves are a transform and a clip-path, so the stylesheet animates it and no JavaScript touches a frame.',
    engine: 'Native',
    dependencies: [],
...
export default function RootLayout({ children }) {
  return (
    ...
    props: [
      ...
      { name: 'depth', type: 'number', def: '0.86', description: 'How far the outgoing page is pushed back, 0 to 1. Smaller is further away.' },
      { name: 'travel', type: 'number', def: '8', description: 'How far the outgoing page travels as it recedes, in viewport units.' },
      { name: 'tilt', type: 'number', def: '6', description: 'Degrees the outgoing page leans away as it drops back. 0 is a flat scale with no depth.' },
    ...
    controls: [
      ...
      { kind: 'range', key: 'depth', label: 'Depth', min: 0.5, max: 1, step: 0.01, def: 0.86 },
      { kind: 'range', key: 'travel', label: 'Travel', min: 0, max: 40, step: 1, def: 8 },
      { kind: 'range', key: 'tilt', label: 'Tilt', min: 0, max: 25, step: 1, def: 6 },
